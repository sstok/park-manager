...
namespace ParkManager\Module\CoreModule\DependencyInjection\Module;

use LogicException;
use ParkManager\Module\CoreModule\DependencyInjection\Module\Traits\ServiceLoaderTrait;
use ReflectionClass;
 ...
 * Services: Resources/config/services/
...
abstract class ParkManagerModuleDependencyExtension extends Extension implements PrependExtensionInterface
{
    ...
    final public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $this->registerRoutes($routeImporter, realpath($this->moduleDir . '/Resources/config') ?: null);

        $loader = $this->getServiceLoader($container, $this->moduleDir . '/Resources/config');
    ...
    {
        $this->initModuleDirectory();
        $resourcesDirectory = $this->moduleDir . '/Resources';
        ...
        if (is_dir($this->moduleDir . '/Resources/translations')) {
            $container->prependExtensionConfig('framework', [
                'translator' => [
                    'paths' => [$this->moduleDir . '/Resources/translations'],
                ],
            ]);
        }
        ...
        if (is_dir($this->moduleDir . '/Resources/templates')) {
            $container->prependExtensionConfig('twig', [
                'paths' => [$this->moduleDir . '/Resources/templates' => $this->getModuleName()],
            ]);
        }
    ...
    final protected function initModuleDirectory(): void
    {
        if ($this->moduleDir === null) {
            ...
            $this->moduleDir = dirname($r->getFileName(), 2);
        }
    }
    ...
    protected function registerWebUIActions(LoaderInterface $loader): void
    {
        ...
        if (file_exists($this->moduleDir . '/Action')) {
            $resolver->registerClasses(
                (new Definition())->setAutowired(true)
                    ->setAutoconfigured(true)
                    ->setPrivate(true),
                $this->moduleNamespace . '\\Action\\',
                $this->moduleDir . '/Action/**/*.php'
            );
        }
    }
}

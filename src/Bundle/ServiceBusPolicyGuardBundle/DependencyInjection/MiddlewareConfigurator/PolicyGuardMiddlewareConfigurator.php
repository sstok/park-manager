...
use Symfony\Component\DependencyInjection\Loader\Configurator\AbstractServiceConfigurator as ServiceConfigurator;
...
final class PolicyGuardMiddlewareConfigurator implements AdvancedMiddlewareConfigurator
{
    ...
    private const RESERVED_VARS = ['message', 'services', 'token'];

    public function __construct(MiddlewaresConfigurator $parent, ServiceConfigurator $di, string $serviceId, ?string $namespacePrefix = '', ?int $priority = -10)
    {
    ...
    public function setVariable(string $name, $value): self
    {
        if (in_array($name, self::RESERVED_VARS, true)) {
            throw new \InvalidArgumentException(sprintf('Variables "%s" are reserved and cannot be overwritten.', implode('", "', self::RESERVED_VARS)));
        }
    ...
    {
        self::assertPolicyIsValid($namespace, $policy);
    ...
    {
        self::assertPolicyIsValid($className, $policy);
    ...
    {
        self::assertPolicyIsValid($regexp, $policy);
        self::assertRegexpIsValid($regexp);
    ...
    private static function assertPolicyIsValid(string $string, $policy): void
    {
        if (! is_bool($policy) && $policy !== null && ! is_string($policy)) {
            throw new \InvalidArgumentException(sprintf('Policy for "%s" must be: boolean, null or a string.', $string));
        }
    }

    private static function assertRegexpIsValid(string $regexp): void
    {
        ...
        try {
            ...
            throw new \InvalidArgumentException(sprintf('Regex policy "%s" cannot be compiled, error: %s', $regexp, mb_substr($e->getMessage(), 14)));
        } finally {
